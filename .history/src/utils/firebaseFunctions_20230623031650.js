...
export const getAllLabReagents = async () => {
  ...
  const labReagentsData = [];

  for (const doc of labReagentsSnapshot.docs) {
    const labReagentData = doc.data();
    const detailsSnapshot = await getDocs(collection(doc.ref, "details"));
    const detailsData = detailsSnapshot.docs.map((doc) => doc.data());

    labReagentData.details = detailsData;
    labReagentsData.push(labReagentData);
  }
