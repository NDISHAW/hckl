...
function LabReagentsComponent({ flag, data }) {
  ...
  const [items, setLabReagents] = useState([]);
  ...
  return (
    ...
    >
      ...
      {filter && (
        <div
          id="basket"
          className="w-full flex items-center justify-start lg:justify-center gap-8 overflow-x-scroll scrollbar-none mt-4"
        >
          {RLA &&
            RLA.find((category) => category.urlParamName === filter)?.subcategories.map(
              (subcategory) => (
                <motion.div
                  key={subcategory.id}
                  className={`group ${
        ...
        <LabContainer
          ...
          data={LabReagents?.filter(
            (item) =>
              ...
              (!subcategoryFilter || item.data.subcategory === subcategoryFilter)
          )}
        />
      </div>
    </div>
...
//           scrollValue={scrollValue}
//           flag={false}
//           data={LabReagents?.filter((item) => item.data.category === filter)}
//         />
//       </div>
//     </div>
//   );
