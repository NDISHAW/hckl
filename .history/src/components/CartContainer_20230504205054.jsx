...
const CartContainer = () => {
  ...
  // };
    ...
    //   axios
    //     .post("http://localhost:5000/api/send-email", { email, cartItems })
    //     .then((response) => {
...
app.post('/send-email', async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      to: toEmail,
      subject,
      html: `<p>${message}</p><table><thead><tr><th>Product</th><th>Price</th><th>Quantity</th></tr></thead><tbody>${cartItems.map(item => `<tr><td>${item.name}</td><td>${item.price}</td><td>${item.quantity}</td></tr>`).join('')}</tbody></table>`,
    });
...
In this example, we're using Nodemailer to send an email using Gmail SMTP service. Note that you will need to provide your Gmail username and password as environment variables to authenticate with Gmail. If you have two-factor authentication enabled for your Gmail account, you will also need to generate an app password and use that instead of your regular Gmail password.
