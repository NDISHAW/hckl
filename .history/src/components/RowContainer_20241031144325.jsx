...
const RowContainer = ({ flag, data,path, scrollValue }) => {
      ...
      {data && data.length > 0 ? (
        data.map((item) => (
          <div
            key={item?.id}
            item={item}
            className="w-275 h-[375px] min-w-[275px] md:w-300 md:min-w-[300px]
            ...
            <div className="w-full flex h-fit items-center justify-between">
              ...
              >
                <img
                  ...
                  onClick={() => navigate(`/productDetails/${item.id}`, { state: { product: item } })}
                  variant="gradient"
                />
              </motion.div>
              {/* Cart */}
