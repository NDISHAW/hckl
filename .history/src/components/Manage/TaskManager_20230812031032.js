...
import AddVet from './Veterenary/AddVet';
function TaskManager({ color }) {
  const [openAddModal, setOpenAddModal] = useState(false);
  const [ openAddLraModal,setOpenAddLraModal] = useState(false);
  const [openAddVetModal, setOpenAddVetModal] = useState(false);
  ...
  //       }, []);
    useEffect(() => {
    const vetColRef = query(
      collection(firestore, "Vet"),
      
    );
    onSnapshot(vetColRef, (snapshot) => {
      setVete(
        snapshot.docs.map((doc) => ({
          id: doc.id,
          data: doc.data(),
  ...
  return (
    <div className="taskManager">
      ...
      <div className="tab-content tab-space">
        ...
        >
          <div className="taskManager__container">
            <button
              className="bg-gradient-to-br bg-blue-300 hover:bg-blue-900 w-flex md:w-auto px-4 py-2  rounded-lg hover:shadow-lg transition-all ease-in-out duration-100"
              onClick={() => setOpenAddVetModal(true)}
            >
              <p className="text-white font-bold text-base md:text-lg m-1">
                <span className="text-xl text-blue-500">
                  <MdAddBox size={60} color="green" />
                  Add VETERENARY products
                </span>
              </p>
            </button>
            <div className="grid">
              {vete && vete.length > 0 ? (
                vete.map((vet) => (
                  <Veterenary
                    id={vet.id}
                    key={vet.id}
                    Method={vet.data.Method}
                    SubCartegory={vet.data.SubCartegory}
                    OrderNumber={vet.data.OrderNumber}
                    Substrate={vet.data.Substrate}
                    DiagnosticApplication={vet.data.DiagnosticApplication}
                    category={vet.data.category}
                    // subcollectionData={task.data.subcollectionData}
                    // category={task.data.category}
                  />
                ))
      ...
      {openAddVetModal && (
        <AddVe
          onClose={() => setOpenAddVetModal(false)}
          open={openAddVetModal}
