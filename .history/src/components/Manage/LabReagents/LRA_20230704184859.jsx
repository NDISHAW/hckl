    ...
    >
      ...
      <div>
        {subcollectionData && (
          <div>
            <p className="mt-12 text-textColor  font-semibold text-base md:text-lg ">
              <span className="text-xl text-blue-500 m-1">Details:</span>
            </p>
            <ul>
              {subcollectionData.map((detail) => (
                <ul>
                  <span className="text-xl text-blue-500 m-1">Details:</span>
                  <li key={detail.id}>{detail.lgClass}</li>
                  <span className="text-xl text-blue-500 m-1">Details:</span><li key={detail.id}>{detail.Format}</li>
                </ul>
              ))}
              {subcollectionData.map((detail) => (
                <ul>
                  {/* <li key={detail.id}>{detail.lgClass}</li> */}
                  {/* <li key={detail.id}>{detail.Format}</li> */}
                </ul>
              ))}
            </ul>
          </div>
