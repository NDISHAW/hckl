...
animate={{ opacity: 1 }}
exit={{ opacity: 0 }}
className={w-full p-2 rounded-lg text-center text-lg font-semibold ${ alertStatus === "danger" ? "text-red-600 bg-red-100" : "text-green-600 bg-green-100" }1}
>
{msg}
</motion.p>
)}
...
{imageAsset && (
<img src={imageAsset} alt="current" className="w-32 h-32 object-cover rounded-lg" />
)}
</div>
<div className="w-full flex flex-col justify-between">
...
<button type="button" onClick={() => handleAddSubData(detail.id)}>
Add Sub Data
</button>
</div>
))}
