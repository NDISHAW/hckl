      ...
      {details && (
        <table>
          ...
          <tbody className="text-xl px-4">
            {details.map((detail) => (
              <tr key={detail.id}>
                <td>{detail.data.gClass}</td>
                <td>{detail.data.Format}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
The details state is initialized as an empty array using the useState hook.
Inside the useEffect hook, the fetchLabReagents function is defined, which fetches data and updates the details state with the fetched data.
The fetchLabReagents function retrieves the LabReagents collection, iterates over the documents, and fetches the subcollection data for each document. It then combines the data and pushes it into the updatedLabReagents array.
The setDetails function is called with the updatedLabReagents array to update the state.
The component renders a table that displays the details data. The table is only rendered if details is truthy.
Inside the tbody, the details array is mapped to generate table rows (<tr>) for each detail object. The key attribute is set to detail.id to ensure a unique key for each row.
The gClass and Format values are accessed from detail.data within each row.
To use this component within the EditLra component using the provided modal, you can include it as follows:
...
const EditLra = ({ onClose, open }) => {
  ...
  return (
    <Modal modalLable="Edit Product" onClose={onClose} open={open}>
      {open && (
        <div>
          {/* Content of the EditLra component */}
          {/* ... */}
          <button onClick={() => setOpenAddModal(true)}>Open Display Component</button>
          {openAddModal && <DisplayDataComponent />}
        </div>
      )}
    </Modal>
  );
...
Here, the EditLra component renders a button that triggers the display of the DisplayDataComponent. The openAddModal state is used to control the visibility of the component. When the button is clicked, setOpenAddModal(true) is called to open the display component, and openAddModal && <DisplayDataComponent /> conditionally renders the DisplayDataComponent based on the openAddModal state.
