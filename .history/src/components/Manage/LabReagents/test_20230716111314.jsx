...
const MyComponent = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const snapshot = await firebase.database().ref('yourCollection').get();
      if (snapshot.exists()) {
  ...
  const addData = (newData) => {
    ...
    firebase.database().ref('yourCollection').push(newData);
  };

  const editData = (id, updatedData) => {
    // Update the existing data in Firebase
    firebase.database().ref(`yourCollection/${id}`).set(updatedData);
  };

  const deleteData = (id) => {
    // Remove the data from Firebase
    firebase.database().ref(`yourCollection/${id}`).remove();
  };
  ...
  return (
    <div>
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault();
          const { id, category, SubCartegory, Method, Species, Substrate, Parameter, lgClass, Format } = e.target.elements;
          addData({
            id: id.value,
            data: {
