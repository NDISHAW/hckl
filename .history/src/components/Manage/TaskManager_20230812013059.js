...
function TaskManager({ color, id }) {
  ...
  const fetchVet = async () => {
    const items = await getDocs(query(collection(firestore, "Vet", id)));
              ...
              {vete && vete.length > 0 ? (
                vete.map((task) => (
                  <Veterenary
                    ...
                    key={task.id}
                    Method={task.Method}
                    SubCartegory={task.SubCartegory}
                    OrderNumber={task.OrderNumber}
                    Substrate={task.Substrate}
                    DiagnosticApplication={task.DiagnosticApplication}
