...
function LabReagentsComponent() {
 ...
 console.log("lab reagents", LabReagents);
  useEffect(() => {
    ...
    onSnapshot(labReagentsColRef, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        const subcollectionRef = collection(doc.ref, ""); // Replace 'SubcollectionName' with the actual subcollection name within each document
  ...
  return (
    <div>
      {abReagents.map((labReagent) => (
        <div key={labReagent.id}>
          <h3>{labReagent.id}</h3>
