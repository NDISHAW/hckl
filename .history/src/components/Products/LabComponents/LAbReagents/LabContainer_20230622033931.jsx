...
function LabContainer({ flag, data, scrollValue }) {
  ...
  return (
    <div
      className={`${
        flag ? "w-full flex justify-center" : "w-1/2 md:w-1/3"
      } px-4 mb-4`}
    >
      <div
        className={`relative shadow-lg w-full rounded-xl bg-white p-4 ${
          scrollValue > 1 ? "animate__animated animate__slideInUp" : ""
        }`}
      >
        ...
        <h4 className="text-lg font-semibold mt-2 text-center">{data.title}</h4>
        <p className="text-sm text-center">{data.description}</p>
        <div className="flex justify-between items-center mt-4">
          <p className="text-lg font-bold text-blue-500">{data.price}</p>
          <button
            className={`rounded-lg px-4 py-1 text-sm font-semibold text-white bg-blue-500 ${
              items.find((item) => item.id === data.id) ? "bg-red-600" : ""
            }`}
            onClick={() =>
          ...
          >
            {items.find((item) => item.id === data.id) ? "Remove" : "Add"}
          </button>
        </div>
      </div>
    </div>
