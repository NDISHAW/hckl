        ...
        >
          {categories &&
            categories.map((category) => (
              ...
              >
                <div
                  className={`w-10 h-395 rounded-full shadow-lg ${
                ...
                <div>                {category.subCartegories?.map((sub) => (
                  <motion.div
                whileTap={{ scale: 0.75 }}
                key={category.id}
                className="w-275 h-[375px] min-w-[275px] md:w-300 md:min-w-[300px]  bg-cardOverlay rounded-lg py-2 px-4  my-35 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-evenly relative">
                    <div
                      className={`text-sm ${
                        filter === category.urlParamName
                          ? "text-white"
