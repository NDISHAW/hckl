...
import { firestore } from "../firebase.config"; // Assuming you have initialized the Firebase app and imported the Firestore instance as 'firestore'
...
function LabReagentsComponent({ flag, data, scrollValue }) {
  const [isOpen, setIsOpen] = useState(false);
  const [labReagents, setLabReagents] = useState([]);
  const labContainer = useRef();

  const [items, setItems] = useState([]);
  const [{ LabReagents, cartShow, cartItems }, dispatch] = useStateValue();
  ...
  useEffect(() => {
    ...
    onSnapshot(labReagentsColRef, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        console.log("lab updatedLabReagents", labReagents);
      });
      console.log("lab updatedLabReagents", labReagents.data);
    });
  ...
  return (
    ...
    >
      {/* <LabContainer
            flag={false}
            data={LabReagents?.labReagents((n) => n.category == labReagents)}
/> */}
      ...
      {labReagents.map((labReagent) => (
        ...
        >
          <div className="w-full flex h-fit items-center justify-between">
            {/* <h3>{labReagent.id}</h3> */}
            {/* <pre>{JSON.stringify(labReagent.data, null, 2)}</pre> */}
            <div className="w-full flex flex-col items-end justify-end -mt-8">
              <p>{labReagent.data.Method}</p>
              <h3>{labReagent.data.category}</h3>
            ...
            >
              ...
              <div
                key={labReagents.data.Parameter}
              ...
              >
                <div className="w-full flex h-fit items-center justify-between">
                  ...
                  >
                    ...
                    <img
                      src={/* Add your image source here */ NotFound}
                      alt=""
                ...
                <div className="w-full flex flex-col items-end justify-end -mt-8">
                  <p className="text-textColor font-semibold text-base md:text-lg">
                    {labReagents.data.Method}
                  </p>
                  <p className="mt-1 text-sm text-gray-500">{labReagents.data.Parameter}</p>
                  <div className="flex items-center gap-8">
                    <p className="text-lg text-headingColor font-semibold">
                      <span className="text-sm text-blue-500">ksh</span>{" "}
                      {labReagents.data.Substrate}
                    </p>
                  </div>
                </div>
              </div>
